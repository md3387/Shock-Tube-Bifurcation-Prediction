function[BifurcationHeight,tI,tA]=MlappPredictBifurcation(ShockFilePath,VacuumFilePath, NumHeaderLines, timeColumn, PressureColumn, PitchColumn, CatchColumn, Mis,Mrs, gamma, MW,D)
...
ax.XLabel.String = 'time [ms]';
%tI
%tILabel = uilabel(fig, 'Text', 'Initial Shock Pressure Rise, t_i [ms]', 'Position', [300 75 200 25]); %Create Label
%tIInput= uieditfield(fig, 'numeric', 'Position', [520 75 50 25], 'Value', 0); %Create input box
%State 2
...
%t_0=tI+D/(2*V_R); %[s] -- See Fig 2, Eq(8), Ref 2 -- %time where bifurcation starts..... Currently ti=user-entered Shock Spike time. time(spike_index)the first step feature due to the deflection of the laser beam - should occur around where pressure trace goes up.  See Ref 2, Fig 2 for visualization.
...
%      He describes his desccription of transition to turbulence as "crude." So, for now, we'll assume all BL follow the M_BL for the laminar BL.
...
theta1Sym = evalin('base', 'theta1Sym'); %<------BUT, these two lines do modify the variable in base workspace, so more careful attention is required.
...
if PBLoverP2<=P5overP2
    ...
    BifurcationHeight_PetersenExtrap=BifurcationHeight_Petersenfit*(15.2/20); %[mm] Specific to 10.16cm Diam USAFA CST, 5mm Kistler PXD, 15.2 mm from endwall -- Should match Eq(5) result.
    BifurcationHeightUncertainty=(BifurcationHeight-BifurcationHeight_PetersenExtrap);% [mm]
    % Note: "The thickness of the boundary layer...was neglected... this extra height should be added"... (Ref 2)
    %...But I haven't found any way to approximate the thickness of the boundary layer.

    %% STEP5(a) (MDH) Calculate remaining Fig1, Ref2 geometry
    AngleOAB=pi()/2; %[rad]=90deg --MDH Assumption. This is notional.
    AngleCOA =theta1; %[rad] --Ref3-- (Davies...See ref below. PDF is on your desktop)
...
try
    ...
    tA %Output is intentionally not supressed.
    t_0 %Output is intentionally not supressed.
    BifurcationHeight %Output is intentionally not supressed.
    message=sprintf('Check your command window for t_A, t_0, and Bifurcation Height. \n The Bifurcation Height Plot failed.\n This typically happens when t_0 > t_A, which causes bifurcation height to be incorrect and negative\n t_0=t_i+TransducerDiameter/(2*Reflected shock Velocity). \n This is likely the combined result of a very small bifurcation combined with offset between the optical plane and pressure transducer center\n The offset may be due to laser misalignment.\n Laser misalignment may also cause the schlieren spikes to drop well below zero in the absorbance plot.');
